...
public class CreateGroupViewModel extends ViewModel {
    ...
    public void createGroup(String title, String description) {
        if (!title.isEmpty() && !description.isEmpty()) {
            mGroupRepository.addGroup(mCookieManager.getCookie(EndPoint.LOGIN), mPreferenceManager.getUser(), mBitmap.getValue(), title, description, Boolean.TRUE.equals(joinType.getValue()) ? "0" : "1", new Callback() {
                @Override
